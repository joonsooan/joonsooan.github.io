...
어떤 형태로 구현하면 좋을지 고민하다 매니저를 만들고 안에 딕셔너리를 만들어서 key : 능력 타입, value : 쿨타임 여부로 만들기로 했어요
...
코루틴을 통해 upgradeData의 cooldownTime 데이터를 가져와 그만큼 쿨타임을 가지게 하고, 해당 key 값의 value를 변경해요
...
이렇게 마무리가 되었으면 좋겠는데, 지금 구현한 로직은 한 가지 문제가 있어요

버튼을 누르는 순간 바로 사용되는 능력들도 있지만, 그렇지 않고 버튼을 누른 이후 화면을 다시 클릭해야지 사용되는 능력들이 있습니다

![Image](https://github.com/user-attachments/assets/9a5bb7ee-b94c-485d-97ea-1ff90865999e)

앞서 구현한 Meteor와 Gravity 능력이 그 예시이죠

따라서 이 경우에도 쿨타임이 적용되도록 코드를 추가해 줘야 합니다

## 쿨타임 구현 2단계

함수를 추가해 주기에 앞서, 앞으로도 이런 능력들이 있을지 모르니 인터페이스를 만들어주면 좋을 것 같습니다
...
이 인터페이스를 상속받는 클래스들을 IsActive, CancelAbility, ActivateAbility를 무조건 가지게 돼요
...
코드를 자세히 들여다보기 전 먼저 구현해 놓은 함수를 살펴볼게요
...
이런 식으로 IsActive를 확인해서 true이면 CancelAbility를, false면 ActivateAbility를 실행하는 메서드입니다

근데 위아래 코드가 스크립트 이름을 빼면 동일한 내용이기 때문에 함수로 만들어주면 좋을 것 같네요

아래는 제네릭 메서드를 추가해서 구현한 코드입니다
...
테스트 과정에서 에러 메시지가 발생했습니다

능력 버튼을 처음 클릭하고 이후에 다시 클릭해서 취소한 다음, 화면을 클릭해 보니 에러가 뜨더라고요

이미 파괴된 게임 오브젝트에 접근하려고 해서 그런 것 같은데, 살펴보니 파괴된 _rangeIndicator 접근하려고 한 것이 원인이었습니다

사실 _rangeIndicator 관련해서 예전에 구현할 때 언젠가 버그가 터질 것 같다고 생각하긴 했는데, 이참에 고쳐야겠습니다
...
ActivateAbility를 else 문 안에 넣어주어 IsActive = false일 때만 실행되도록 해주면 문제 해결입니다 👍
...
한 가지 문제점?은 현재 cooldownTime(쿨다운 시간)이 controlTime(능력 시간)보다 짧은 경우 버그가 발생합니다

일단 임시방편으로 무조건 cooldownTime이 controlTime보다 길도록 데이터를 입력한 상태인데, 지금 상황에서 어떤 버그가 또 발생할지 예측이 안 돼서 이대로 진행할 계획이에요
